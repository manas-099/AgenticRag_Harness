...
from rag_harness.infrastructure.llm import GroqFallbackClient, MyLLMClient
...
def get_rag_pipeline() -> RAGPipeline:
    ...
    myllm_client = MyLLMClient(get_myllm_settings())
    groq_client = GroqFallbackClient(get_groq_settings())
    embedder = SentenceTransformerEmbedder(embedding_settings)
    model_registry = CrossEncoderRegistry(get_rerank_settings())
    ...
    chunker = ContextualChunker(groq_client, get_chunking_settings(), splitter)
    ...
    section_filter_llm = SectionFilterLLM(myllm_client)
    metadata_filter_service = MetadataFilterService(section_selector, section_filter_llm)

    generate_use_case = GenerateAnswerUseCase(myllm_client, cache, get_retrieval_settings())

    claim_extractor = ClaimExtractor(myllm_client)
    ...
    decision_engine = AgentDecisionEngine(myllm_client)
    loop_settings = get_harness_loop_settings()
    compiled_graph = build_harness_graph(agent_tools, decision_engine, generate_use_case, verify_use_case, myllm_client, loop_settings)
